...
impl DeliveryService {
    ...
    pub async fn deliver_activity(&self, inbox_url: &str, activity: Value) -> Result<()> {
        ...
        let response = self.client
            ...
            .header("User-Agent", format!("Fediverse-Node/{}", env!("CARGO_PKG_VERSION")))
            .json(&activity)
            .send()
            .await?;
        ...
        if response.status().is_success() {
            ...
            warn!("Failed to deliver activity to {}: {}", inbox_url, response.status());
    ...
    pub async fn deliver_to_followers(&self, activity: Value, followers: Vec<String>) -> Result<()> {
    ...
    pub async fn deliver_to_public(&self, activity: Value, public_inboxes: Vec<String>) -> Result<()> {
