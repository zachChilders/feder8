...
pub async fn get_outbox(
    ...
    let total_items = match db.get_actor_outbox_count(&actor.id).await {
        Ok(count) => count,
        Err(e) => {
            warn!("Database error while counting outbox items for {}: {}", username, e);
            return Ok(HttpResponse::InternalServerError().json(serde_json::json!({
                "error": "Internal server error"
            })));
        }
    ...
    let activities = match db.get_activities_by_actor(&actor.id, 20, 0).await {
        ...
        Err(e) => {
            warn!("Database error while fetching outbox activities for {}: {}", username, e);
            return Ok(HttpResponse::InternalServerError().json(serde_json::json!({
                "error": "Internal server error"
            })));
        }
    ...
    let activity_objects: Vec<Value> = activities.into_iter()
        .map(|activity| {
            serde_json::json!({
                "id": activity.id,
                ...
                if let Some(object) = activity.get("object") {
                    if let Some(object_type) = object.get("type").and_then(|v| v.as_str()) {
                        if object_type == "Note" {
                            ...
                            // Generate unique IDs
                            let activity_id = format!("{}/activities/{}", config.server_url, uuid::Uuid::new_v4());
                            let note_id = format!("{}/notes/{}", config.server_url, uuid::Uuid::new_v4());
                            
                            // Extract note data
                            let content = object.get("content").and_then(|v| v.as_str()).unwrap_or("").to_string();
                            let to_recipients = activity.get("to").and_then(|v| v.as_array())
                                .map(|arr| arr.iter().filter_map(|v| v.as_str().map(|s| s.to_string())).collect())
                                .unwrap_or_else(Vec::new);
                            let cc_recipients = activity.get("cc").and_then(|v| v.as_array())
                                .map(|arr| arr.iter().filter_map(|v| v.as_str().map(|s| s.to_string())).collect())
                                .unwrap_or_else(Vec::new);
                            ...
                            activity_object["attributedTo"] = serde_json::Value::String(actor.id.clone());
