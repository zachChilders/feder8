...
impl ActivityPubServer {
    ...
    pub fn register_handlers(&self, server: &mut EspHttpServer) -> Result<()> {
        ...
        server.fn_handler(
            ...
            move |request| Self::handle_webfinger(request, container.clone()),
        )?;
        ...
        server.fn_handler(
            ...
            move |request| Self::handle_inbox(request, container.clone()),
        )?;
        ...
        server.fn_handler(
            "/users/*/outbox",
            esp_idf_svc::http::Method::Get,
            move |request| Self::handle_outbox_get(request, container.clone()),
        )?;
        ...
        server.fn_handler(
            "/users/*/outbox",
            esp_idf_svc::http::Method::Post,
            move |request| Self::handle_outbox_post(request, container.clone()),
        )?;
        ...
        server.fn_handler(
            "/nodeinfo/2.0",
            esp_idf_svc::http::Method::Get,
            move |request| Self::handle_nodeinfo(request, container.clone()),
        )?;
    ...
    /// Mock register handlers for non-ESP32 environments
    #[cfg(not(target_arch = "xtensa"))]
    pub fn register_handlers(&self, _server: &mut ()) -> Result<()> {
        info!("Mock ActivityPub HTTP handlers registered successfully");
        Ok(())
    }
    ...
    #[cfg(target_arch = "xtensa")]
    fn handle_webfinger(
        request: Request<&mut EspHttpServer>,
        container: Arc<Mutex<EmbeddedContainer>>,
    ) -> Result<()> {
        ...
        let container = container
            .lock()
            .map_err(|e| anyhow::anyhow!("Failed to lock container: {}", e))?;
    ...
    #[cfg(target_arch = "xtensa")]
    fn handle_inbox(
        mut request: Request<&mut EspHttpServer>,
        container: Arc<Mutex<EmbeddedContainer>>,
    ) -> Result<()> {
        ...
        match activity.activity_type.as_str() {
            ...
            _ => {
                info!(
                    "Received unknown activity type: {}",
                    activity.activity_type.as_str()
                );
            }
        }
    ...
    fn handle_health_check(
        ...
        let health_response = json!({
            ...
            "free_heap": crate::utils::get_free_heap_size(),
            "uptime_ms": crate::utils::get_timer_ms()
        });
...
mod tests {
    ...
    fn test_container_access() {
        ...
        assert_eq!(
            container_ref.actor().preferred_username.as_str(),
            "testuser"
        );
    }

    #[test]
    fn test_register_handlers_mock() {
        let container = Arc::new(Mutex::new(create_test_container()));
        let server = ActivityPubServer::new(container);

        // Test that mock register handlers works
        let result = server.register_handlers(&mut ());
        assert!(result.is_ok());
    }
}
