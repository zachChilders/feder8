...
fn test_cargo_toml_structure() {
    ...
    assert!(
        cargo_content.contains("name = \"feder8-firmware\""),
    ...
    assert!(
        cargo_content.contains("esp-idf-sys"),
        "Missing esp-idf-sys dependency"
    );
    assert!(
        cargo_content.contains("esp-idf-hal"),
        "Missing esp-idf-hal dependency"
    );
...
fn test_qemu_simulation() {
    ...
    assert!(
        stdout.contains("Hello, ESP32 World!"),
        "Missing hello world message"
    );
    assert!(
        stdout.contains("LED pin configured successfully"),
        "Missing LED config message"
    );
    assert!(
        stdout.contains("LED ON") || stdout.contains("LED OFF"),
        "Missing LED toggle message"
    );
...
fn test_source_code_syntax() {
    ...
    let main_content = std::fs::read_to_string("src/main.rs").expect("Failed to read main.rs");

    // Basic syntax checks
    assert!(main_content.contains("fn main()"), "Missing main function");
    assert!(
        main_content.contains("esp_idf_sys"),
        "Missing ESP-IDF imports"
    );
    assert!(main_content.contains("println!"), "Missing println macro");

    // Check sim_runner.rs
    let sim_content =
        std::fs::read_to_string("src/sim_runner.rs").expect("Failed to read sim_runner.rs");

    assert!(
        sim_content.contains("fn main()"),
        "Missing main function in sim_runner"
    );
    assert!(
        sim_content.contains("Command"),
        "Missing Command import in sim_runner"
    );
