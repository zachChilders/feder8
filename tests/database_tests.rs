use chrono::Utc;
use feder8::database::{create_configured_mock_database, Database, DatabaseRef, DbActor, DbActivity, DbFollowRelation, DbNote, MockDatabase};
...
async fn test_mock_database_activity_operations() {
    ...
    let activities = db.get_activities_by_actor(&test_actor_id, 20, 0).await.unwrap();
...
async fn test_mock_database_note_operations() {
    ...
    let notes = db.get_notes_by_actor("https://example.com/users/testuser", 20, 0).await.unwrap();
...
async fn test_mock_database_follow_operations() {
    ...
    let followers = db.get_followers("https://example.com/users/bob", 20, 0).await.unwrap();
    assert_eq!(followers.len(), 1);
    assert_eq!(followers[0].follower_id, "https://example.com/users/alice");
    
    // Test getting following
    let following = db.get_following("https://example.com/users/alice", 20, 0).await.unwrap();
    assert_eq!(following.len(), 1);
    assert_eq!(following[0].following_id, "https://example.com/users/bob");
    
    // Test getting follower count
    let count = db.get_actor_followers_count("https://example.com/users/bob").await.unwrap();
    assert_eq!(count, 1);
    
    // Test getting following count
    let count = db.get_actor_following_count("https://example.com/users/alice").await.unwrap();
    assert_eq!(count, 1);
    
    // Test updating follow status
    db.update_follow_status(&test_follow.id, "accepted").await.unwrap();
}
...
async fn test_database_with_complex_data() {
    ...
    let note = db.get_note_by_id("https://example.com/notes/complex").await.unwrap();
    assert!(note.is_some());
    let note = note.unwrap();
